...
if [ "$BuildTarget" == "linux" ]; then
  ...
  DropboxSymbolsPath="/media/psf/Dropbox/Ansible/symbols"
  if [ ! -d "$DropboxSymbolsPath" ]; then
    DropboxSymbolsPath="/mnt/c/Ansible/Dropbox/Ansible/symbols"
    if [ ! -d "$DropboxSymbolsPath" ]; then
  ...
  if [ ! -d "/media/psf/backup/tdesktop" ]; then
    BackupPath="/mnt/c/Ansible/Projects/backup/tdesktop/$AppVersionStrMajor/$AppVersionStrFull/$BackupFolder"
    if [ ! -d "/mnt/c/Ansible/Projects/backup/tdesktop" ]; then
      Error "Backup folder not found!"
    fi
  fi

  ./build/docker/centos_env/run.sh /usr/src/tdesktop/Ansible/build/docker/build.sh
...
if [ "$BuildTarget" == "mac" ] || [ "$BuildTarget" == "macstore" ]; then
  ...
  DropboxSymbolsPath="$HOME/Dropbox/Ansible/symbols"
  if [ ! -d "$DropboxSymbolsPath" ]; then
    Error "Dropbox path not found!"
  fi
  ...
  if [ "$NotarizeRequestId" == "" ] || [ "$NotarizeRequestId" == "go" ]; then
    ...
    if [ "$BuildTarget" == "mac" ]; then
      codesign --force --deep --timestamp --options runtime --sign "Developer ID Application: Telegram FZ-LLC (C67CF9S4VU)" "$ReleasePath/$BundleName" --entitlements "$HomePath/Ansible/Ansible.entitlements"
    elif [ "$BuildTarget" == "macstore" ]; then
      codesign --force --timestamp --options runtime --sign "3rd Party Mac Developer Application: Telegram FZ-LLC (C67CF9S4VU)" "$ReleasePath/$BundleName/Contents/Frameworks/Breakpad.framework/Versions/A/Resources/breakpadUtilities.dylib" --entitlements "$HomePath/Ansible/Breakpad.entitlements"
      codesign --force --deep --timestamp --options runtime --sign "3rd Party Mac Developer Application: Telegram FZ-LLC (C67CF9S4VU)" "$ReleasePath/$BundleName" --entitlements "$HomePath/Ansible/Ansible Lite.entitlements"
