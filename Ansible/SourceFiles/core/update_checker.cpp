...
bool checkReadyUpdate() {
	...
	// Ansible.app (OUTPUT_NAME в Ansible/CMakeLists.txt). Литерал Telegram.app
...
void UpdateApplication() {
	if (UpdaterDisabled()) {
		const auto url = [&] {
#ifdef OS_WIN_STORE
			return "https://desktop.ansible.su";
#elif defined OS_MAC_STORE // OS_WIN_STORE
			return "https://desktop.ansible.su";
#else // OS_WIN_STORE || OS_MAC_STORE
			if (KSandbox::isFlatpak()) {
				return "https://flathub.org/apps/details/com.ansible.desktop";
			} else if (KSandbox::isSnap()) {
				return "https://desktop.ansible.su";
			}
			return "https://ansible.su";
