...
[[nodiscard]] QString TgMediaSource(
	...
	if (kind == RichPage::BlockKind::Photo) {
		return photoId
			? u" data-tg-src=\"as://photo?id=%1\""_q.arg(photoId)
			: QString();
	} else if (!documentId) {
		return QString();
	}
	return u" data-tg-src=\"as://%1?id=%2\""_q.arg(
...
[[nodiscard]] EntityTags EntityTagsFor(
	...
	case EntityType::Mention:
		return LinkTags(u"https://asme.su/"_q
	...
	case EntityType::MentionName: {
		...
		return fields.userId
			? LinkTags(u"as://user?id=%1"_q.arg(fields.userId))
			: EntityTags();
	}
...
void SerializeBlock(
	...
	case Kind::Channel: {
		const auto link = block.username.isEmpty()
			? QString()
			: u"https://asme.su/"_q + block.username;
