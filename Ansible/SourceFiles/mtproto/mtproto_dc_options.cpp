...
// Ansible bootstrap — встроенный DC ровно один.
//
// Прод-СПб: ws.ansible.su -> 85.193.80.91, порт 10443 (не 443).
// Остальные DC клиент подхватит сам через dcOptions, как только
// help.getConfig их объявит. Все четыре таблицы (обычная, IPv6,
// test, IPv6-test) держат один и тот же адрес: отдельного тестового
// окружения нет, а пустая таблица травит кеш DC у клиента.
...
// 🚨 Это ключ РУКОПОЖАТИЯ с DC (RSA-2048). В проекте есть ВТОРОЙ
// кастомный RSA-ключ — проверка подписи обновлений, RSA-1024, в
// config.h. Их легко перепутать: оба наши, оба RSA, лежат рядом.
//
// Тестовый и боевой совпадают: отдельного тестового DC у нас нет.
