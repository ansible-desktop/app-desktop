...
2. Choose an icon file name:
   ...
   - Match existing naming conventions (check `Ansible/Resources/icons/{subfolder}/`)
...
4. Write `.ai/icon_{name}/context.md` with:
   ...
   ## Icon: {icon_name}
   Type: {menu/other}
   Target: Ansible/Resources/icons/{subfolder}/{icon_name}.svg
...
if [[ "$OSTYPE" == darwin* ]]; then
    ls out/Ansible/codegen/codegen/style/Debug/codegen_style
else
    ls out/Ansible/codegen/codegen/style/Debug/codegen_style.exe
fi
```
...
```bash
CODEGEN=$(if [[ "$OSTYPE" == darwin* ]]; then echo out/Ansible/codegen/codegen/style/Debug/codegen_style; else echo out/Ansible/codegen/codegen/style/Debug/codegen_style.exe; fi)
$CODEGEN --render-svg Ansible/Resources/icons/menu/tag_add.svg .ai/icon_{name}/test_render.png 512
```
...
2. Copy the final SVG to that target path (e.g., `Ansible/Resources/icons/menu/{icon_name}.svg`).
