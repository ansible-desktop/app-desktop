...
QString Translator::translate(const char *context, const char *sourceText, const char *disambiguation, int n) const {
	if (u"QMenuBar"_q == context) {
		...
		if (u"Hide %1"_q == sourceText) return tr::lng_mac_menu_hide_ansible(tr::now, lt_telegram, u"%1"_q);
		...
		if (u"Quit %1"_q == sourceText) return tr::lng_mac_menu_quit_ansible(tr::now, lt_telegram, u"%1"_q);
		if (u"About %1"_q == sourceText) return tr::lng_mac_menu_about_ansible(tr::now, lt_telegram, u"%1"_q);
		return QString();
	}
