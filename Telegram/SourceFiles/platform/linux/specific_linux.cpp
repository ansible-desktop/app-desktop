...
bool GenerateDesktopFile(
	...
	const auto sourceFile = u":/misc/com.ansible.desktop.desktop"_q;
	...
	if (!Core::UpdaterDisabled()) {
		...
		if (!Core::Launcher::Instance().customWorkingDir()) {
			QFile::remove(u"%1com.ansible.desktop._%2.desktop"_q.arg(
				targetPath,
				md5Hash));
		...
		QFile::remove(u"%1com.ansible.desktop.%2.desktop"_q.arg(
			targetPath,
			md5Hash));
	}
...
bool GenerateServiceFile(bool silent = false) {
	...
	if (!Core::UpdaterDisabled()
		...
		QFile::remove(u"%1com.ansible.desktop._%2.service"_q.arg(
			targetPath,
			md5Hash));
	}
...
void start() {
	QGuiApplication::setDesktopFileName([&] {
		...
		if (!Core::UpdaterDisabled()) {
			return u"com.ansible.desktop._%1"_q.arg(
				Core::Launcher::Instance().instanceHash().constData());
		}

		return u"com.ansible.desktop"_q;
	}());
